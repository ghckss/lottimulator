...
import { readDataRecommendation, readLottoHistory } from "@/features/lotto/history";

export async function HomeContent() {
  const [lottoHistory, dataRecommendation] = await Promise.all([
    readLottoHistory(),
    readDataRecommendation()
  ]);
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6">
      <section className="min-w-0 space-y-5">
        ...
        <LottoMachine draws={lottoHistory} dataRecommendation={dataRecommendation} />
      </section>
    </main>
  );
}
